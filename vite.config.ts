...
const PRODUCTION_URL = 'https://tg-client.crmchat.ai';
...
export default defineConfig(({ mode }): UserConfig => {
  ...
  const defaultAppTitle = `CRMchat TG Client${appEnv !== 'production' ? ' Beta' : ''}`;
...
function buildCsp(appEnv: string) {
  return `
  default-src 'self';
  connect-src 'self' wss://*.dc.crmchat.ai wss://*.dcx.crmchat.ai wss://*.ru-dc.crmchat.ai blob: http: https: ${appEnv === 'development' ? 'wss: ws: ipc:' : ''};
